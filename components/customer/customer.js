...
.controller('customerCtrl', ['$scope', '$http', function($scope, $http) {
	...
	$http.get('http://localhost:8080/leadfleet-api/routeDetails').
	...
	$scope.deleteRoute = function (idx) {
		$http({
			method: 'DELETE',
			url: 'http://localhost:8080/leadfleet-api/routeDetails/'+$scope.listOfRoute[idx].id,
		}).success(function (data) {
	...
	$scope.createRouteDetail = function () {
		...
		$http({
			method: 'POST',
			url: 'http://localhost:8080/leadfleet-api/routeDetails',
	...
	$scope.updateRouteDetail = function () {
		$http({
			method: 'PUT',
			url: 'http://localhost:8080/leadfleet-api/custom/routeDetails/' + $scope.routeDetail.id,
			data: JSON.stringify($scope.routeDetail),
			headers: { 'Content-Type': 'application/JSON' }
